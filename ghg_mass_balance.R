...
mb <- dat %>%
  select(date, group, site, distance_m,
...
for(i in 1:length(dates)){
  dd_out <- mb %>%
    ...
    select(date, site, reach_no, gas, discharge, distance_m, depth_m, length_m, 
           sa_m2, c_out_ugl = concentration_ugl,csat_out_ugl = sat_ugl,Fout_gd, 
           Met_gO2m2d, K_d)
  dd_in <- mb %>%
  ...
  dd <- left_join(dd_out, dd_in, by = c('reach_no', 'gas')) %>%  
    ...
    select(date, site, distance_m, reach_no, length_m, discharge, sa_m2, 
