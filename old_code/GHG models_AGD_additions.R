...
find_best_model <- function(preds, y, gas, flux = FALSE){
    ...
    w <- which(mods$rsq_pred >= arrange(mods, -rsq_pred)$rsq_pred[10])
    w <- unique(c(w, which(mods$aicc <= arrange(mods, aicc)$aicc[10])))
    mods <- mods[w,] %>%
...
best_mods <- bind_rows(CO2.mods, CO2flux.mods,
                       CH4.mods, CH4flux.mods,
                       N2O.mods, N2Oflux.mods,
                       O2.mods, O2flux.mods)
