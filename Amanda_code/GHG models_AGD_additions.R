...
car#setwd("C:/Users/Alice Carter/Dropbox (Duke Bio_Ea)/projects/ghg_patterns_nhc/")
setwd('C://Users/adelv/Dropbox/Duke/NHC/Alice')


dat <- read_csv("ghg_flux_complete_drivers_dataframe.csv")
...
preds <- scaled %>%
  dplyr::select(logQ, watertemp_C, GPP, ER, slope_nhd, DO.obs, 
...
# CO2
mm <- lm(CO2.ugL ~ logQ + watertemp_C + slope_nhd + GPP + ER + DO.obs +
           doc_mgl, data = scaled)
...
lme0site<-lme(CO2.ugL~1, random=~1|site, data=scaled)
summary(lme0site)
VarCorr(lme0site)
...
#I like to step through models manually because sometimes a really small drop in the AIC 
#involves removing a variable close to significant
...
AIC(mm) #52.529
#try removing doc first
vif(mm) #I haven't used this before and got an error - not  sure how to use this properly

mmdoc <- lm(CO2.ugL ~ logQ + watertemp_C + slope_nhd + GPP + ER + DO.obs +
            site, data = scaled)
anova(mmdoc)
summary(mmdoc)
AIC(mmdoc) #55.567
...
mmsite<-lm(CO2.ugL ~ logQ + watertemp_C + slope_nhd + GPP + ER + DO.obs +
             doc_mgl, data = scaled)
AIC(mmsite) #59.6
#also worse

#maybe last try removing discharge
mmdsc<-lm(CO2.ugL ~ watertemp_C + slope_nhd + GPP + ER + DO.obs +
            doc_mgl + site, data = scaled)
AIC(mmdsc)
anova(mmdsc)
#very slight improvement to 51.818 - not enough to actually remove it
summary(mmdsc)
cov(preds)
#however recall that DOC and discharge are quite correlated
plot(doc_mgl~logQ, data=scaled)
...
#if date wont be added in (which I don't *think* it needs to be since temp especially is here)
#then prob an important point is dicussing driver change over time - which I think you
#do already, just stressing it
...
#and then I see that ER was removed when you ran it with the function (based on 
#table in figures) - so trying that

mmdsc.ER<-lm(CO2.ugL ~ watertemp_C + slope_nhd + GPP + DO.obs +
            doc_mgl + site, data = scaled)
AIC(mmdsc.ER)
AIC(mmdsc) #a drop of 1 pt, not necesarily better
summary(mmdsc.ER) #only loses 0.2 pct of the variance explained soooo guess its worth removing
#also saw slope was removed 
#however, also think it might be worth revisiting the base models with interactions

mmdsc.ER.slope<-lm(CO2.ugL ~ watertemp_C  + GPP + DO.obs +
                             doc_mgl + site, data = scaled)
AIC(mmdsc.ER.slope) #is something wrong here?
AIC(mmdsc.ER) #a drop of 1 pt, not necesarily better
summary(mmdsc.ER.slope)



#going back to base model with interaction
mm_int <- lm(CO2.ugL ~ logQ*doc_mgl + watertemp_C + slope_nhd + GPP + ER + DO.obs +
           + site, data = scaled)
AIC(mm_int) #54 - okay interaction is worse

# CH4
mm <- lm(CH4.ugL ~ logQ + watertemp_C + slope_nhd + GPP + ER + DO.obs +
...
# N2O
mm <- lm(N2O.ugL ~ logQ + watertemp_C + slope_nhd + GPP + ER +
...
mm <- lm(N2O.flux_ugld ~ logQ + watertemp_C + slope_nhd + GPP + ER +
           DO.obs + no3n_mgl + doc_mgl, data = scaled)
...
# O2
mm <- lm(DO.obs ~ logQ + watertemp_C + slope_nhd + GPP + ER + 
           doc_mgl, data = scaled)
  out <- get_mod_results(mm, gas = "O2")
  rsqs <- bind_rows(rsqs, out$r2)
  mod_steps <- bind_rows(mod_steps, out$steps)
  mod_fits <- bind_rows(mod_fits, out$mod)
mm <- lm(O2.flux_ugld ~ logQ + watertemp_C + slope_nhd + GPP + ER + 
           doc_mgl, data = scaled)
