...
ss <- sf::st_as_sf(sites, coords = c('longitude', 'latitude'),
               crs = 4326)
sites$slope_deg <- raster::extract(ssc, ss)
ggplot(sites[1:7,], aes(slope_wbx, slope_deg, col = sitecode)) +
    geom_point(size = 3)
write_csv(sites, 'C:/Users/alice.carter/git/ghg_patterns_nhc/data/site_data/NHCsite_metadata.csv')
